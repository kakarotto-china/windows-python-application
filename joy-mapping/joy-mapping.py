...
def reCheckJoy(pygame):
    ...
    if len(joysticks) == 0:
        ...
        global AUTO_START
        AUTO_START = 1 # 未检测到手柄, 全局变量AUTO_STAR置为1, 返回后, 则在主菜单中开始检测, 连接后继续调用start方法
        return False
...
def start():
    ...
    while reCheckJoy(pygame): # 实时检测手柄连接状态
        pygame.time.Clock().tick(60)  # 通过时钟对象指定循环频率 每秒循环60次

        for event in pygame.event.get():  # 监听用户事件
            if event.type == pygame.QUIT:  # 判断用户是否点击了关闭按钮
                print("回到主菜单 [joy-mapping程序] 退出...")
                global AUTO_START
                AUTO_START = 2 # 退出时AUTO_START置为2, 用来返回到主菜单
...
def getKeyboard(msg):
    ...
    posMsg = ((500 - fontMsg.get_width()) / 2, 20)
    while doWhile:
        pygame.time.Clock().tick(60)  # 通过时钟对象指定循环频率 每秒循环60次
        ...
        for event in pygame.event.get():  # 监听用户事件
            ...
            if event.type == pygame.KEYDOWN:
                ...
                posKeyStr = ((500 - fontKeyStr.get_width()) / 2, 100)
...
def doMap():
    ...
    for k, v in MAPPING.items("mapping"):
        # keyName = 'None'
        # while not VK_CODE.__contains__(keyName) and keyName != '':
        #     print("输入键盘命令字符, 映射到 [" + k + "] 操作(按[Enter]跳过/输入quit终止)")
        #     keyName = input(">>>")
        #     if keyName == 'quit':
        #         return
        # MAPPING.set("mapping", k, keyName)
...
def joyCode():
    ...
    # print("\033[33m*** 按下手柄按键, 显示对应的手柄码, 可手动修改 [binding.ini] 配置文件 ***\033[0m")
    printYellow("*** 按下手柄按键, 显示对应的手柄码, 可手动修改 [binding.ini] 配置文件, 例: UP = btn-1 (按下组合键[Ctrl+C]回到主菜单) ***")
    # print("*** 按下手柄按键, 显示对应的手柄码, 可手动修改 [binding.ini] 配置文件 ***")
    ...
    while doWhile:
        ...
        for event in pygame.event.get():  # 监听用户事件
            ...
            if event.type == pygame.JOYAXISMOTION:  # joy 0 type ... hat/axis/button
                ...
                pass
            if event.type == pygame.JOYBALLMOTION:  # 追踪球
                pass
            if event.type == pygame.JOYBUTTONDOWN:  # event.button => A 0, B 1, X 2, Y 3, LEFT 4, RIGHT 5, SELECT 6, START 7, AXIS_LEFT 8, AXIS_RIGHT 9
                print("btn", event.button, sep="-")
...
def main_menu():
    ...
    while True:
        ...
        while 0 == AUTO_START or 2 == AUTO_START:
            # printYellowBG("-" * 10 + "手柄映射工具" + "-" * 10)
            print()
            # printGreenBG("手柄映射工具")
            # print()
            print("★ " * 15)
    ...
    while True: # 循环检测
        ...
        if len(joysticks) == 0:
            if len(testting) >= 6:
                testting = ""
            testting += " ."
            # print("\r\033[31m请连接手柄" + testting + "\033[0m", end="")
            printRed("\r请连接手柄" + testting, end="")
            # print("\r请连接手柄" + testting, end="")
        else:
            print("\r", end="")
            for joystick in joysticks:
                # joystick.init()
                # \033[31m这是红色字体\033[0m
                # print("\033[32m " + joystick.get_name(), "\033[0m 手柄已连接", "设备id:", joystick.get_id())
                printGreen(str(joystick.get_name()))
                print("手柄已连接", "设备id:", str(joystick.get_id()))
                # print(" " + joystick.get_name(), "手柄已连接", "设备id:", joystick.get_id())
            break # 检测到则退出循环
        pygame.joystick.quit()
...
def init():
    ...
    定义一些全局变量
        ...
        AUTO_START: 自动启动, 初始值为0, 在启动菜单0后, 手柄断开则会将值置为1, 则会自动启动, 退出菜单0后则会置为2, 继续显示主菜单页面
    :return:
    '''
    ...
    global VK_CODE, VK_CODE_PYGAME, MAPPING, BINDING, PATH_MAPPING, PATH_BINDING, PATH_JOY_PNG, INTERVAL, AUTO_START
